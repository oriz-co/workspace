...
description: "Every login command and dashboard URL needed to publish the @chirag127 packages and deploy the 11 oriz-family sites. Run these on YOUR terminal. Tokens go into envpact (the vault); none of them are pasted into agent chats. Re-run any section when a token is rotated."
tags: [runbook, auth, secrets, publish, deploy]
...
```bash
cd /c/D/oriz/projects/oriz/own/prod/apps/personal/oriz-cs-me-app
