...
- [Loop engineering — how agent loops should be structured](./loop-engineering.md) — ReAct (Thought-Action-Observation) is the default loop. Tool-call budgets per class (read-only 50, mutating 10, sub-agent spawn 5). Halt conditions are explicit, not emergent (goal-met, budget-exceeded, repeated-state, tool-fails-3x, user-interrupt, context >80%). Observation summarization for outputs >200 lines / binary / repeated. Context-shrink on 2nd repeat failure, halt + grill on 3rd. PEVR for multi-file architectural work; evaluator-optimizer for quality-driven tasks.
