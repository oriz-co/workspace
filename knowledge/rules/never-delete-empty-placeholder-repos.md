...
description: "An empty repo in the chirag127/oriz* family is a deliberate slug reservation, NOT a cleanup candidate. Before running gh repo delete (or any destructive remote action) against any family repo, even an empty one, the agent MUST get an explicit, repo-named confirmation from the user."
tags: [rule, repo, delete, placeholder, slug-reservation, family, safety]
...
- **Outward-effect actions are already gated** by
  [`rules/push-by-default.md`](./push-by-default.md) — `gh repo
...
- [Push-by-default rule](./push-by-default.md) — outward-effect carve-out this rule narrows
- [Repo naming rule](./repo-naming.md) — audit before publish; audit before delete is the symmetric ask
- [Rename repo runbook](../runbooks/rename-repo.md) — the right way to retire a slug (rename, don't delete)
