...
description: Compute work is split across GitHub Actions cron (build-time), Cloudflare Workers (edge runtime), and the user's browser. Each tier has a free quota and a clear remit.
tags: [architecture, compute, workers, github-actions, browser, layer-5]
...
- ONE Hono Worker at `api.oriz.in` (see [api-umbrella-hono-worker.md](api-umbrella-hono-worker.md))
