---
type: architecture
title: Submodule pattern — each site/package/extension is a separate GitHub repo
description: Every site, every package, and every extension is a standalone GitHub repo added as a git submodule under sites/, packages/, or extensions/. The submodule has its own commits, releases, CI, and main branch. The master oriz repo stores a SHA pointer per submodule.
tags: [architecture, git, submodules, repo, workflow]
...
- What the master pointer represents → [master-pointer-as-production-sha.md](master-pointer-as-production-sha.md)
- The branching rule → [`../rules/one-branch-only.md`](../rules/one-branch-only.md)
