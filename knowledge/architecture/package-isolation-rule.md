...
description: Every external service must be wrapped in a typed @chirag127 package so swapping providers is a package version bump, not a 50-file rewrite. Any new service crossing 3+ sites' boundary gets a wrapper on first introduction.
tags: [architecture, packages, isolation, swap-cost, rule]
...
- Server-side mirror: routes under `apps/api/src/routes/` are folder-per-service so an integration swap is contained — see [api-routes-structure.md](api-routes-structure.md).
...
- The current packages → [the-17-packages.md](the-17-packages.md) (17 packages locked)
