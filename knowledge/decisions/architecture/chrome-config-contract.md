...
description: "Locked: generic components driven by 4 per-site config files; 3-level sidebar (Section → Group → Leaf); shared Datasheet Dark tokens across every site (no per-site accent); Iosevka wordmark stamp (slug-only, no ORIZ prefix); 24 auto-generated legal pages; pnpm workspace at the workspace umbrella root."
...
- `projects/npm-packages/*` (8 astro-* packages)
...
- [decisions/architecture/sidebar-4-tier](./sidebar-4-tier.md) — predecessor (4-tier preset categories, refined here to 3-level depth per node)
- [decisions/architecture/a11y-three-tools](./a11y-three-tools.md)
- [decisions/architecture/multi-engine-search-button](./multi-engine-search-button.md) — MultiSearch component spec
