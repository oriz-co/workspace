---
type: decision
title: "Lifestream auto-event sources — three streams (GitHub webhooks + Wakatime daily + CF Web Analytics daily)"
description: "Locked 2026-06-20: the oriz-me JSONL canonical store is fed by THREE auto-tracked event sources only — GitHub webhooks via Hookdeck, Wakatime daily-summary cron, and Cloudflare Web Analytics daily-summary cron. No manual entry, no minute-grain coding capture, no per-pageview visitor capture. Reinforces the auto-only-tracking rule."
tags: [lifestream, jsonl, auto-tracking, github-webhooks, wakatime, cloudflare-web-analytics, decisions, architecture]
...
related:
  ...
  - decisions/architecture/cron-split-cf-vs-gh
...
The [oriz-me JSONL canonical store](./lifestream-jsonl-canonical.md)
...
file growth at the [JSONL canonical](./lifestream-jsonl-canonical.md)
"~10 MB/year" envelope.
...
- **Health-check coverage** — both daily crons ping
  ...
  success per
  [`health-check-cron-plus-uptime`](./health-check-cron-plus-uptime.md);
  miss → alert.
...
- **Schema** lives in `chirag127/oriz-me-data/schema.json`; the
  [JSONL canonical decision](./lifestream-jsonl-canonical.md) covers
  the validation flow.
- **Forward refs**: ingest workers under `api.oriz.in/lifestream/*`
  belong to the [umbrella Hono Worker](./hono-worker-api-umbrella.md)
...
- [Lifestream JSONL canonical decision](./lifestream-jsonl-canonical.md)
- [Lifestream federation (AT Protocol + ActivityPub mirrors)](./lifestream-federation.md)
- [Cron split — CF Cron vs GH Actions](./cron-split-cf-vs-gh.md)
