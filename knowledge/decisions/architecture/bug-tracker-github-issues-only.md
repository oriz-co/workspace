...
description: "Locked 2026-06-20: every site / extension / package / worker / data repo uses its own GitHub Issues as the sole bug tracker. Linear, Trello, Jira, Plane.so, Asana, Height — all REJECTED. Cross-repo triage via repo:org searches. Free unlimited, GitHub-native, integrates with PRs and commits via #123 syntax."
tags: [decisions, architecture, bug-tracker, github-issues, intake]
...
- **GitHub-native integrations** — `#123` in a commit message
  ...
  [`code-quality-five-tools`](./code-quality-five-tools.md) and in
...
- **Free unlimited** — GitHub Issues has no per-issue cap, no
  ...
  posture (per [`rules/repos-work-independently`](../../rules/repos-work-independently.md))
  means external contributors can file bugs without needing a
  Linear / Jira seat.
- **One-branch-only fits naturally.** Per
  [`rules/one-branch-only`](../../rules/one-branch-only.md) +
...
| Tool | Why rejected |
|---|---|
| Linear | Free tier 250 issues / 2 users — fights [`never-hit-quotas`](../../rules/never-hit-quotas.md); paid tier requires card. Adds a second tracker the family has to keep in sync with GitHub PRs |
...
- `good-first-issue` — kept; the everything-is-public-OSS posture
  per [`repos-work-independently`](../../rules/repos-work-independently.md)
...
- [One-branch-only rule](../../rules/one-branch-only.md)
- [Repos work independently rule](../../rules/repos-work-independently.md)
- [Conventional commits rule](../../rules/conventional-commits.md) — commit messages reference `#123`
- [No card-on-file rule](../../rules/no-card-on-file.md)
- [No subscriptions rule](../../rules/no-subscriptions.md)
- [Code quality 5-tool stack](./code-quality-five-tools.md) — CodeRabbit + Sonarcloud surface findings as issue-style comments
