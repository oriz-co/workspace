...
| Per-user compute (auth-gated, requires fresh data) | **Client-side fetch + Firestore REST direct from browser** | Spark Firestore: 50K reads/day, 20K writes/day | [`firebase-rest-firestore-not-admin`](./firebase-rest-firestore-not-admin.md) |
...
- [`hono-worker-api-umbrella.md`](./hono-worker-api-umbrella.md): unchanged. Hono Workers were already the chosen umbrella API layer; this decision just rules out the Functions alternative explicitly.
- [`cron-split-cf-vs-gh.md`](./cron-split-cf-vs-gh.md): unchanged. CF Workers cron + GH Actions cron continue to split the cron load.
...
- [Firestore via REST not Admin SDK](./firebase-rest-firestore-not-admin.md)
