...
Books are written in `.md` files using **Markua-flavoured Markdown** (a Leanpub-friendly Markdown superset). The new npm package [`@chirag127/oriz-book-build`](https://www.npmjs.com/package/@chirag127/oriz-book-build) (v0.1.0, 17th family package) wraps **Pandoc** to emit **EPUB3 + PDF + MOBI** from `manuscript/*.md` into `dist/`. On every tag push (`v*.*.*`) in a book repo, [`@chirag127/omni-publish`](./omni-publish-package.md) takes the dist artefacts and fans out to 5 channels:
...
Repo slug: `chirag127/<slug>-book` (matches family `<role>-suffix` convention per [`repo-naming-suffixes`](../branding/repo-naming-suffixes.md)). On-disk path: `projects/books/<slug>-book/`.
...
- **Repo**: `chirag127/oriz-stack-book` → `projects/books/oriz-stack-book/`
...
- **Repo**: `chirag127/oriz-paisa-book` → `projects/books/oriz-paisa-book/`
...
- **Repo**: `chirag127/oriz-pdf-book` → `projects/books/oriz-pdf-book/`
...
- **Repo**: `chirag127/oriz-janaushdhi-book` → `projects/books/oriz-janaushdhi-book/`
...
- **Repo**: `chirag127/oriz-me-book` → `projects/books/oriz-me-book/`
