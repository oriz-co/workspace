...
description: "Image origin storage uses a 4-tier chain — repo-hosted on CF Pages → ImgBB → Imgur → GitHub user-content. Composes alongside the 3-tier image-CDN chain in the oriz-kit <Image> wrapper."
tags: [images, host, origin, fallback, oriz-kit, never-hit-quotas]
...
[3-tier image-CDN chain](./image-cdn-fallback-chain.md) that handles
...
- **`<Image>` wrapper composes** origin chain (this decision) with
  CDN chain (the [existing decision](./image-cdn-fallback-chain.md)) — sites stay unaware of either chain.
- **Storage caveat.** This decision governs **images only**.
  ...
  [`object-storage-split.md`](./object-storage-split.md). Different
...
- [3-tier image-CDN chain](./image-cdn-fallback-chain.md) — delivery layer composed alongside this origin chain
- [Markdown-in-repo only CMS decision](./cms-markdown-in-repo-only.md) — authoring story behind Tier 1
- [Object-storage split](./object-storage-split.md) — different asset class (binaries / blobs vs. images)
