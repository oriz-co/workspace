...
NEW app `oriz-data-aggregator-app` at `c:/D/oriz/projects/apps/content/oriz-data-aggregator-app/`. Hosted on Cloudflare Pages (one-level subdomain — Universal SSL works).
...
`@chirag127/auth-core` package gains a `<SignInButton />` component that always redirects to `https://auth.oriz.in/sign-in?return=<current-url>`. The `oriz-auth-app` (NEW; at `projects/apps/hub/oriz-auth-app/`) hosts the sign-in UI itself, mounted at `auth.oriz.in`.
