...
| Repo naming suffix matrix — `-site` / `-ext` / `-vsc-ext` / `-cli` / `-worker` / `-fn` / `-data`; npm packages stay clean | [`knowledge/decisions/branding/repo-naming-suffixes.md`](./knowledge/decisions/branding/repo-naming-suffixes.md) |
